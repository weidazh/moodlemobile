...
MM.settings = {
    ...
    addFakeNotifications: function(e, setting) {
        ...
        if (MM.plugins.notifications) {
            setTimeout(function() {
                MM.plugins.notifications.saveAndDisplay(data);
            }, 5000);
        }
    ...
    languageSelected: function(e, setting) {
        ...
        MM.log("vcvg: languageSelected newLang" + newLang);
        MM.setConfig("lang", newLang);
        MM.log('vcvg: loading newLang.json: lang/' + newLang + '.json');
        $.getJSON('lang/' + newLang + '.json', function(langFile) {
            MM.log('vcvg: language json load ok');

            MM.log('vcvg: MM.lang.loadLang');
