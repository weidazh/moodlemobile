...
MM.lang = {
    ...
    determine: function() {
        // User preferences.
        MM.log("vcvg: checking language setting");
        var lang = MM.getConfig('lang');
        if (typeof(lang) != 'undefined') {
            MM.log("vcvg: language setting is saved: " + lang);
            return lang;
        }

        // Locale
        // MM.lang.locale is loaded by Phonegap.
        MM.log("vcvg: checking MM.lang.locale setting");
        if (MM.lang.locale) {
            lang = MM.lang.locale.toLowerCase().replace("-", "_");
            MM.log("vcvg: MM.lang.locale =" + lang);
            if (typeof(MM.config.languages[lang]) != "undefined") {
                MM.log("vcvg: found" + lang);
                return lang;
            } else if(lang.length > 2) {
                // Try without the region/country.
                MM.log("vcvg: Try without the region/country " + lang);
        ...
        MM.log("vcvg: Try browser lang " + lang);
        if (typeof(MM.config.languages[browserLang]) != "undefined") {
            MM.log("vcvg: found " + browserLang);
            return browserLang;
        ...
        if (typeof(MM.config.current_site) != 'undefined' &&
            ...
            MM.log("vcvg: Try current site setting " + MM.config.current_site.lang);
            return MM.config.current_site.lang;
        }

        MM.log("vcvg: Try default_lang " + MM.config.default_lang);
        // Default language.
